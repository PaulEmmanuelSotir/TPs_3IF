	...
	for (uf32 i = NK; i < NB*(NR + 1); i++) { // Erreur dans les indices !!
		...
		if (i%NK == 0) {
			tmp = (T4[(u8)(tmp >> 24)] & 0x000000FF)
				^ (T4[(u8)(tmp >> 16)] & 0xFF000000)
				^ (T4[(u8)(tmp >> 8)] & 0x00FF0000)
				^ (T4[(u8)(tmp)] & 0x0000FF00)
				^ rcon[i / NK];
		}
#if NK > 6
		if (i%NK == 4) {
			tmp = (T4[(u8)(tmp >> 24)] & 0xFF000000)
				^ (T4[(u8)(tmp >> 16)] & 0x00FF0000)
				^ (T4[(u8)(tmp >> 8)] & 0x0000FF00)
				^ (T4[(u8)(tmp)] & 0x000000FF);
		}
#endif
		Key_ex[i] = Key_ex[i - NK] ^ tmp;
	}
}
...
void init_rand(u32* key, u32* plain, const size_t nk, const size_t nb, int params)
{
	...
	for (i = 0; i < nk; i++)
		key[i] = ((rand() + rand() * 7 + 13 * i + 3216546541 * i)) << i ^ ((rand() + rand() * 7 + 13 * i + 3216546541 * i)) >> i + params & 0xFFFFFFFF;
...
u32 AES(u32* Px, const u32* Kex)
{
	...
	memcpy(R1x, Px, NB*sizeof(u32));
	memxor(R1x, Kex);
	Round(R2x, R1x, Kex + NB);
	Round(R1x, R2x, Kex + 2 * NB);
	Round(R2x, R1x, Kex + 3 * NB);
	Round(R1x, R2x, Kex + 4 * NB);
	Round(R2x, R1x, Kex + 5 * NB);
	Round(R1x, R2x, Kex + 6 * NB);
	Round(R2x, R1x, Kex + 7 * NB);
	Round(R1x, R2x, Kex + 8 * NB);
	Round(R2x, R1x, Kex + 9 * NB);
	Round(R1x, R2x, Kex + 10 * NB);
